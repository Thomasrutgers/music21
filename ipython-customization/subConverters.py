# -*- coding: utf-8 -*-
#Edited by Thomas to make the show('midi') work in iPython
#
#-------------------------------------------------------------------------------
...
#         return HTML(htmlBlock + js)
    ...
    def show(self, obj, fmt, app=None, subformats=None, **keywords): # pragma: no cover
        ...
        if helperFormat in ('musicxml', 'xml', 'lilypond', 'lily'):
            ...
            encoded_audio = self._get_audio_as_base64(fp)
            if encoded_audio is not None:
                self._display_audio_tag(encoded_audio)

        ### PRIVATE METHODS ###


    def _display_audio_tag(self, encoded_audio):
        ...
        #audio_tag = """<audio controls src='file:///tmp/music21/out.wav'>"""
        #display_html(audio_tag, raw=True)
    ...
    def _get_audio_as_base64(
        ...
        command = 'timidity {midi_file_path} -Ow --output-24bit -o /tmp/music21/out2.wav'.format(
            midi_file_path=midi_file_path,
            )
        result = subprocess.call(command, shell=True)
        if result == 0:
            encoded_audio = self._get_base64_from_file('/tmp/music21/out2.wav')
